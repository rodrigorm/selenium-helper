 ...
 */
class SeleniumShell extends Shell {

/**
 * The test category, "app" or the name of a plugin
 *
 * @var string
 * @access public
 */
 ...
 */
	function parseArgs() {
		...
		if (!in_array($this->category, array('app'))) {
 ...
 */
	function __canRun() {
		$notApp = !in_array($this->category, array('app'));
		$isPlugin = in_array(Inflector::underscore($this->category), $this->plugins);

		if ($notApp && !$isPlugin) {
			$message = sprintf(
				__('%s is an invalid test category (either "app" or name of a plugin)', true),
				$this->category
			);
			$this->error($message);
			return false;
